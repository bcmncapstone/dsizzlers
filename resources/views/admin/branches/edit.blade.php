...
<div class="py-6">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white shadow-sm p-8 rounded-lg mb-6">
            <h1>Edit Contract</h1>
            <p class="header-subtitle">Update contract location and franchisee details</p>
        </div>
        ...
        <!-- Form -->
        <form id="branchForm" action="{{ route('admin.branches.update', $branch->branch_id) }}" method="POST" enctype="multipart/form-data" class="bg-white shadow-sm p-8 rounded-lg">
            @csrf
            @method('PUT')
            ...
            <div class="form-group">
                ...
                <!-- Helper Text -->
                <p class="field-help">Use the Address field below or click on the map to set the location. Drag the marker to adjust position.</p>

                <!-- Address Input & Suggestions (Shopee-like) -->
                <div style="margin-top:12px; position:relative;">
                    <label class="form-label">Address</label>
                    <input type="text" name="location_address" id="location_address" class="form-input" placeholder="Search address or drag the pin" value="{{ old('location', $branch->location) }}">
            ...
            <div class="form-group">
                ...
                @php
                    $matchedFranchisee = isset($franchisees) ? $franchisees->firstWhere('franchisee_email', $branch->email) : null;
                    $selectedFranchiseeId = old('franchisee_id', optional($matchedFranchisee)->franchisee_id);
                @endphp
                <select id="franchisee_select" name="franchisee_id" class="form-input" onchange="fillFranchiseeInfo(this)">
                    ...
                    @isset($franchisees)
                        @foreach($franchisees as $fr)
                            <option value="{{ $fr->franchisee_id }}" data-name="{{ $fr->franchisee_name }}" data-email="{{ $fr->franchisee_email }}" data-contact="{{ $fr->franchisee_contactNo }}" {{ (string) $selectedFranchiseeId === (string) $fr->franchisee_id ? 'selected' : '' }}>{{ $fr->franchisee_name }} — {{ $fr->franchisee_email }}</option>
                        @endforeach
                    @endisset
                </select>
            ...
            <div class="form-group">
                ...
                <div class="file-input-wrapper">
                    <input type="file" name="contract_file" id="contract_file" class="file-input" accept="application/pdf,.pdf">
                ...
                @if ($branch->contract_file)
                    <p class="field-help">Current file: {{ basename($branch->contract_file) }}. Upload a new file only if you want to replace it.</p>
                @endif
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const currentContractFilename = @json($branch->contract_file ? basename($branch->contract_file) : null);
    ...
    function updateContractLabel() {
        ...
        } else if (currentContractFilename) {
            contractFilename.textContent = `Current file: ${currentContractFilename}`;
        } else {
